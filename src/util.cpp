...
const char * const BITCOIN_CONF_FILENAME = "eziocoin.conf";
const char * const BITCOIN_PID_FILENAME = "eziocoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "eziocoin" is a composite category enabling all Eziocoin-related debug output
            if(ptrCategory->count(string("eziocoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EziocoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EziocoinCore
    // Mac: ~/Library/Application Support/EziocoinCore
    // Unix: ~/.eziocoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EziocoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EziocoinCore";
#else
    // Unix
    return pathRet / ".eziocoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty eziocoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override eziocoin.conf
